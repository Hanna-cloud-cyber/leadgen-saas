...
This is a standalone Next.js app — the homepage (`/`) IS the Sculptia product page. It renders with a
hardcoded demo catalog (`src/app/data.ts`) until you connect a real
...
they're unset, the homepage keeps rendering the built-in demo catalog and
...
same environment variables there. Once deployed, the site is live and
wired to your store.

## 5. What's still manual in Shopify

- **Bundle pricing** ("Buy 2, Get 1 FREE" etc.): the page shows these as
  marketing copy (`bundles` in `src/app/data.ts`); the discount
...
- `src/app/data.ts` — `getSculptiaProduct()` fetches the live
  product server-side and falls back to the hardcoded catalog if Shopify
  isn't configured or the request fails.
- `src/app/page.tsx` — server component, fetches the product and
  renders the client UI.
- `src/app/SculptiaClient.tsx` — all the interactive UI (color/size/
  bundle selection), unchanged visually from before.
- `src/app/api/cart/route.ts` — creates a Shopify cart for the
